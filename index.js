...
app.use(rateLimit({ windowMs: 60 * 1000, max: 500, standardHeaders: true, legacyHeaders: false,
...
function nullPaths(value, prefix, out, depth) {
  ...
  if (value === null) { out.push(prefix || "<root>"); return out; }
  if (typeof value !== "object") return out;
  if (Array.isArray(value)) {
    for (let i = 0; i < value.length && i < 5; i++) nullPaths(value[i], `${prefix}[${i}]`, out, depth + 1);
    return out;
  }
  for (const k of Object.keys(value)) {
    nullPaths(value[k], prefix ? `${prefix}.${k}` : k, out, depth + 1);
  }
  return out;
}
...
app.use((req, res, next) => {
  try {
    ...
    const entry = {
      ...
      body: (req.body && Object.keys(req.body).length) ? JSON.stringify(req.body).slice(0, DIAG_MAX_BODY) : "",
      status: 0, ms: 0, resp: "", nulls: [], err: "",
    };

    req._diag = entry;
    _diagReqs.push(entry);
    if (_diagReqs.length > DIAG_MAX_ENTRIES) _diagReqs.shift();

    res.on("finish", () => {
      try { entry.status = res.statusCode; entry.ms = Date.now() - started; } catch (e) {}
    });

    res.on("close", () => {
      try { if (!entry.status) { entry.status = -1; entry.err = "client closed"; entry.ms = Date.now() - started; } } catch (e) {}
    });
    ...
    res.json = (payload) => {
      ...
      } catch (e) { entry.resp = "<unserialisable: " + e.message + ">"; }
    ...
    res.send = (payload) => {
      try {
        if (!entry.resp) entry.resp = (typeof payload === "string" ? payload : JSON.stringify(payload)).slice(0, DIAG_MAX_RESP);
      } catch (e) {}
...
app.post("/debug/note", (req, res) => {
  try {
    ...
    if (_diagNotes.length > 400) _diagNotes.shift();
  } catch (e) {}
...
app.get("/debug/requests", requireAdminKey, (req, res) => {
  ...
  if (req.query.url) rows = rows.filter(x => x.url.includes(String(req.query.url)));
  if (req.query.since) rows = rows.filter(x => x.t >= String(req.query.since));
  if (req.query.status) rows = rows.filter(x => String(x.status) === String(req.query.status));

  const limit = Math.max(1, Math.min(300, parseInt(req.query.limit) || 50));
  rows = rows.slice(-limit);

  if (req.query.slim) rows = rows.map(({ t, m, url, status, ms, nulls }) => ({ t, m, url, status, ms, nulls }));

  res.status(200).json({ total: _diagReqs.length, returned: rows.length, reqs: rows });
});
app.get("/debug/clear", requireAdminKey, (req, res) => { _diagReqs.length = 0; res.status(200).send("cleared"); });

app.get("/debug/notes", requireAdminKey, (req, res) => {
  const limit = Math.max(1, Math.min(400, parseInt(req.query.limit) || 100));
  res.status(200).json({ total: _diagNotes.length, notes: _diagNotes.slice(-limit) });
});
app.get("/debug/notes/clear", requireAdminKey, (req, res) => { _diagNotes.length = 0; res.status(200).send("cleared"); });

app.use(require("./RolesModule"));
app.get("/shared", sendShared);

const ASSETS_REPO = process.env.ASSETS_REPO || "ccperfectshot11/StumbleGuysCache";

app.get("/assets/manifest", async (req, res) => {
  try {
    if (!process.env.GITHUB_TOKEN) {
      return res.status(503).json({ error: "assets not configured" });
    }
    ...
    const relRes = await fetch(`https://api.github.com/repos/${ASSETS_REPO}/releases/latest`, { headers });
    if (!relRes.ok) return res.status(502).json({ error: `github ${relRes.status}` });

    const release = await relRes.json();
    const asset = (release.assets || []).find((a) => a.name.endsWith(".zip"));
    if (!asset) return res.status(502).json({ error: "no zip in latest release" });
    ...
    if (!url) return res.status(502).json({ error: "no signed url" });

    res.status(200).json({ version: release.tag_name, url, sizeBytes: asset.size });
  } catch (err) {
...
app.get("/user/steampricesv2", (req, res) => res.status(200).json({}));

app.post("/party/update", (req, res) => {
  try {
    const { action, username, id, code } = req.body || {};
    console.log(`[Party] ${username || "?"} (${id || "?"}) ${action || "update"}${code ? " code=" + code : ""}`);
  } catch (e) {}
  res.status(200).send("OK");
});

const AC_IMMUNE_ROLES = ["dev", "owner"];
async function isAntiCheatImmune(deviceId) {
  try {
    if (!deviceId) return false;
    const u = await UserModel.findByDeviceId(deviceId);
    return !!(u && AC_IMMUNE_ROLES.includes(u.role));
  } catch (e) { return false; }
}

app.post("/anticheat/report", async (req, res) => {
  try {
    const { stumbleId, deviceId, reason, key, hwids } = req.body || {};

    if (process.env.ACKey && key !== process.env.ACKey) {
      return res.status(403).json({ error: "bad key" });
    }
    if (!stumbleId && !deviceId) {
      return res.status(400).json({ error: "stumbleId or deviceId required" });
    }

    let user = null;
    if (stumbleId) user = await UserModel.findByStumbleId(stumbleId);
    if (!user && deviceId) user = await UserModel.findByDeviceId(deviceId);
    if (!user) {
      Console.log("AntiCheat", `Report with no matching user (stumbleId=${stumbleId}, deviceId=${deviceId}, reason=${reason})`);
      return res.status(404).json({ error: "user not found" });
    }

    if (await isAntiCheatImmune(user.deviceId)) {
      Console.log("AntiCheat", `IMMUNE (privileged) ${user.username} — report ignored (reason: ${reason || "?"})`);
      return res.status(200).json({ status: "immune" });
    }

    await UserModel.update(user.stumbleId, {
      isBanned: true,
      banReason: `[AntiCheat] ${reason || "cheat detected"}`,
      bannedAt: new Date(),
    });
    ...
    try {
      if (Array.isArray(hwids)) {
        for (const hw of hwids) {
          if (typeof hw === "string" && hw.length > 0) await UserModel.addHwid(user.stumbleId, hw);
        }
      }
    } catch (e) {}
    ...
    try {
      if (user.deviceId) {
        UserController.bannedDevices.set(user.deviceId, { until: Date.now() + 3650 * 24 * 60 * 60 * 1000, bannedAt: Date.now() });
      }
    } catch (e) {}

    Console.log("AntiCheat", `BANNED ${user.username} (stumbleId=${user.stumbleId}) reason: ${reason || "?"}, hwids: ${Array.isArray(hwids) ? hwids.length : 0}`);
...
app.post("/anticheat/check", async (req, res) => {
  try {
    const { stumbleId, deviceId, hwids, key } = req.body || {};
    ...
    if (stumbleId || deviceId) {
      let me = null;
      if (stumbleId) me = await UserModel.findByStumbleId(stumbleId);
      if (!me && deviceId) me = await UserModel.findByDeviceId(deviceId);

      if (me && await isAntiCheatImmune(me.deviceId)) {
        return res.status(200).json({ banned: false, immune: true });
      }

      if (me && me.isBanned) {
        return res.status(200).json({ banned: true, immune: false, reason: me.banReason || "Banned" });
      }
    }

    const bannedOwner = await UserModel.findBannedByHwids(hwids);
    if (bannedOwner) {
      Console.log("AntiCheat", `Evasion detected: machine of banned ${bannedOwner.username} used by stumbleId=${stumbleId || "?"}`);

      if (stumbleId) {
        const me = await UserModel.findByStumbleId(stumbleId);
        if (me) {
          await UserModel.update(me.stumbleId, {
            isBanned: true,
            banReason: `[AntiCheat] evasion (shares hardware with ${bannedOwner.username})`,
            bannedAt: new Date(),
          });
          try {
            if (Array.isArray(hwids)) for (const hw of hwids) if (typeof hw === "string" && hw.length > 0) await UserModel.addHwid(me.stumbleId, hw);
          } catch (e) {}
          try { if (me.deviceId) UserController.bannedDevices.set(me.deviceId, { until: Date.now() + 3650 * 24 * 60 * 60 * 1000, bannedAt: Date.now() }); } catch (e) {}
        }
      }
      return res.status(200).json({ banned: true, reason: `evasion (shares hardware with ${bannedOwner.username})` });
    }

    return res.status(200).json({ banned: false, immune: false });
  } catch (err) {
    Console.error("AntiCheat", "Error:", err);
...
app.get("/photon/auth", VerifyPhoton);
...
app.get('/economy/offers/purchasedV2/', (req, res) => res.status(200).json({}));

app.get('/cosmetic-sources-config', (req, res) => res.status(200).json([]));

app.get('/collection-events/me', (req, res) => res.status(200).json({ CollectionEvents: [] }));

function pusherConfigured(res) {
  if (process.env.PUSHER_KEY && process.env.PUSHER_SECRET) return true;
  res.status(503).json({ error: "pusher not configured" });
  return false;
}

function pusherAuth(stringToSign) {
  const sig = crypto.createHmac("sha256", process.env.PUSHER_SECRET).update(stringToSign).digest("hex");
...
function pusherTrigger(channel, event, data) {
  return new Promise((resolve) => {
    ...
    if (!appId || !key || !secret) {
      Console.error("Pusher", "trigger skipped: PUSHER_APP_ID/KEY/SECRET not set");
      _diagNotes.push({ t: new Date().toISOString(), who: "pusher", text: `SKIP ${event}->${channel}: missing ${!appId?"APP_ID ":""}${!key?"KEY ":""}${!secret?"SECRET":""}` });
    ...
    const req = https.request(
      {
        ...
        port: 443,
        method: "POST",
        path: `${path}?${qs}&auth_signature=${sig}`,
      ...
      (res) => {
        ...
        res.on("data", (c) => (out += c));
        res.on("end", () => {
          _diagNotes.push({ t: new Date().toISOString(), who: "pusher", text: `${event}->${channel} HTTP ${res.statusCode} ${String(out).slice(0,120)}` });
          if (res.statusCode >= 200 && res.statusCode < 300) resolve(true);
          else {
            Console.error("Pusher", `trigger ${event} -> HTTP ${res.statusCode}: ${out}`);
            resolve(false);
          }
        });
      }
    ...
    req.on("error", (e) => {
      Console.error("Pusher", `trigger ${event} error: ${e.message}`);
      _diagNotes.push({ t: new Date().toISOString(), who: "pusher", text: `ERR ${event}->${channel}: ${e.message}` });
      resolve(false);
    });
    req.write(body);
    req.end();
...
app.post('/pusher/authorize', (req, res) => {
  if (!pusherConfigured(res)) return;

  const { channel_name, socket_id } = req.body || {};
  if (!channel_name || !socket_id) {
    return res.status(400).json({ error: "missing channel_name or socket_id" });
  }
  ...
  res.status(200).json({ auth: pusherAuth(`${socket_id}:${channel_name}`) });
});

app.post('/pusher/authenticate', (req, res) => {
  if (!pusherConfigured(res)) return;

  const { socket_id } = req.body || {};
  if (!socket_id) return res.status(400).json({ error: "missing socket_id" });

  const userData = JSON.stringify({ id: String(req.user.id) });

  res.status(200).json({
    auth: pusherAuth(`${socket_id}::user::${userData}`),
    user_data: userData,
  });
});
...
app.delete('/user/creator-codes', CreatorCodeController.stopSupport);
app.delete('/user/creator-codes/:code', CreatorCodeController.stopSupport);

app.get('/admin/creator-codes/list', CreatorCodeController.getList);
...
app.post('/party/invite', async (req, res) => {
  try {
    ...
    const { UserIds, PhotonAppId, PhotonRoomCode, PhotonRegion, EventId } = req.body || {};
    if (!Array.isArray(UserIds) || UserIds.length === 0 || !PhotonRoomCode) {
      return res.status(400).json({ error: "missing UserIds or PhotonRoomCode" });
    }
    ...
    const payload = {
      ...
      PhotonRegion: PhotonRegion || "",
      PhotonAppId: PhotonAppId || "",
      PartyContext: EventId ? { "game-event-id": EventId } : {},
    };

    await Promise.all(
      UserIds.map((targetId) =>
        pusherTrigger(`private-user-${targetId}`, "stumble:v0:party_invite", payload)
      )
    );
...
app.post('/game/recently-played-with', (req, res) => res.status(200).json({ ok: true }));
...
app.delete("/tournamentx/:tournamentId/leave", TournamentXController.leave);
...
app.use((err, req, res, next) => {
  try { if (req._diag) req._diag.err = String((err && err.stack) || err).slice(0, 1500); } catch (e) {}
  next(err);
});
